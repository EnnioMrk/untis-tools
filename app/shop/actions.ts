...
export async function openThemeCheckout(
    themeId: ShopThemeId,
    ...
    if (theme.priceEuroCents === 0) {
        return {
            success: false,
            error: "The default theme is already included.",
        };
    }
    ...
    try {
        ...
        console.error(
            `Failed to open checkout for ${theme.name} (${formatEuroCents(theme.priceEuroCents)}):`,
            error,
        );
        return { success: false, error: "Failed to start checkout." };
    }
...
export async function activateOwnedTheme(
    themeId: ShopThemeId,
