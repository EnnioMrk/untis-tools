...
export async function getUserStats(
    ...
    if (Array.isArray(userStats.dailyTrend)) {
        dailyTrend = (
            userStats.dailyTrend as unknown as Array<{
                date: string;
                absenceRate: number;
                totalLessons?: number;
                absences?: number;
...
/**
 * Get fresh user stats - designed for client-side fetching after date change
 */
export async function getFreshUserStats(): Promise<UserStatsResponse | null> {
    return getUserStats(false);
}

/**
 * Get the data start date for the current user
 * Returns the custom date if set, otherwise returns the school year start date
 */
export async function getDataStartDate(): Promise<{
    date: Date | null;
    isCustom: boolean;
}> {
    const session = await auth();
    if (!session?.user?.id) {
        return { date: null, isCustom: false };
    }
    ...
    if (connection?.dataStartDate) {
        return { date: connection.dataStartDate, isCustom: true };
    }

    // Return school year start as default
    const schoolYearStart = await getSchoolYearStart();
    return { date: schoolYearStart, isCustom: false };
}

/**
 * Update the data start date for the current user
 * Setting null will use the school year start date
 * @param dateString - ISO date string or null
 */
export async function updateDataStartDate(
    ...
    try {
        ...
        await prisma.untisConnection.update({
            where: { userId: session.user.id },
            data: { dataStartDate: date },
        });

        // Invalidate cache so stats are recalculated with new date range
        invalidateUserStatsCache(session.user.id);

        // Trigger a sync with the new date range
...
/**
 * Get preset date options based on school holidays
 */
export async function getPresetDateOptions(): Promise<
...
> {
    ...
    try {
        const res = await fetch(
            `https://openholidaysapi.org/SchoolHolidays?countryIsoCode=DE&subdivisionCode=DE-NI&languageIsoCode=DE&validFrom=${year}-01-01&validTo=${year + 1}-12-30`,
        );
        ...
        const filteredHolidays = data
            ...
            .filter(
                (h: { endDate: string }) => new Date(h.endDate) < new Date(),
            )
            .sort(
        ...
        const presets: { label: string; date: Date }[] = filteredHolidays.map(
            (h: { name: { text: string }[]; endDate: string }) => {
                ...
                return {
                    label: `${holidayName} (${new Date(h.endDate).getFullYear()})`,
                    date: startDate,
                };
            },
        );
