...
export async function createAdminReferralCode(formData: FormData) {
  ...
  if (!parsed.success) {
    throw new Error(parsed.error.errors[0]?.message || 'Invalid referral code payload');
  }
...
export async function createCouponCode(formData: FormData) {
  ...
  if (!parsed.success) {
    throw new Error(parsed.error.errors[0]?.message || 'Invalid coupon payload');
  }

  const normalizedCode = normalizeCode(parsed.data.code);
  const code = normalizedCode || normalizeCode(`COUPON-${crypto.randomUUID().slice(0, 8)}`);
...
export async function updateUserSubscription(formData: FormData) {
  ...
  await prisma.user.update({
    ...
    data: {
      ...
      planSource: planSource as 'NONE' | 'SUBSCRIPTION' | 'TRIAL' | 'BONUS',
      trialEndsAt: planSource === 'TRIAL' ? new Date(Date.now() + 1000 * 60 * 60 * 24 * 30) : null,
      accessEndsAt: planSource === 'BONUS' ? new Date(Date.now() + 1000 * 60 * 60 * 24 * 30) : null,
    },
  });
