...
const SHOP_THEME_ID_SET = new Set<ShopThemeId>(
    SHOP_THEMES.map((theme) => theme.id),
);

export function normalizeShopTheme(
...
export function getShopTheme(
    ...
    return (
        SHOP_THEMES.find((entry) => entry.id === normalizeShopTheme(theme)) ||
        SHOP_THEMES[0]
    );
}
