...
import { APP_PLANS, type AppPlan, type PaidPlan, type PlanConfig } from './types';
...
export function isPlanAtLeast(currentPlan: string | null | undefined, requiredPlan: AppPlan): boolean {
  return PLAN_ORDER.indexOf(normalizePlan(currentPlan)) >= PLAN_ORDER.indexOf(requiredPlan);
}

export function canAccessWidget(plan: string | null | undefined, widgetType: WidgetType): boolean {
...
export function getRequiredPlanForWidget(widgetType: WidgetType): AppPlan {
  return (
    PLAN_ORDER.find((plan) => PLAN_CONFIGS[plan].features.allowedWidgetTypes.includes(widgetType)) ||
    'PREMIUM'
  );
}
