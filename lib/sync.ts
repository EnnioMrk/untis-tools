...
export async function triggerImmediateSync(userId: string): Promise<{
  ...
  try {
    ...
    if (!connection) {
      console.error('[triggerImmediateSync] No Untis connection found for user:', userId);
      return { success: false, error: 'No Untis connection found' };
    }

    if (!connection.isActive) {
      console.error('[triggerImmediateSync] Untis connection is not active for user:', userId);
      return { success: false, error: 'Untis connection is not active' };
    }

    console.log('[triggerImmediateSync] Connection found, server:', connection.serverUrl, 'school:', connection.school);
    ...
    await untis.login();
    console.log(`[triggerImmediateSync] Logged in to Untis for user ${userId}`);

    // Calculate date range - use school year start for full year data
    const endDate = new Date();
    const startDate = await getSchoolYearStart();
    console.log('[triggerImmediateSync] Date range:', startDate.toISOString(), 'to', endDate.toISOString());
    ...
    try {
      ...
      timetable = await untis.getOwnTimetableForRange(startDate, endDate) || [];
      console.log('[triggerImmediateSync] Fetched timetable, lessons count:', timetable.length);
    } catch (error) {
      console.error('[triggerImmediateSync] Failed to fetch timetable:', error);
    }
    ...
    try {
      ...
      console.log('[triggerImmediateSync] Fetched absences, count:', absences.length);
    } catch (error) {
      console.error('[triggerImmediateSync] Failed to fetch absences:', error);
    }

    console.log(`[triggerImmediateSync] Fetched ${timetable.length} lessons and ${absences.length} absences`);
    ...
    const previousStatsData: UserStatsData | null = previousStats?.subjectBreakdown as any;
    console.log('[triggerImmediateSync] Previous stats:', previousStats ? 'exists' : 'none');

    // Calculate new statistics
    const stats = calculateStats(timetable, absences, previousStatsData);
    console.log('[triggerImmediateSync] Calculated stats:', JSON.stringify(stats.absenceCounts));

    // Convert subjectBreakdown from Record to Array format
    const subjectBreakdownArray = Object.entries(stats.subjectBreakdown).map(([subject, data]) => ({
      subject,
      total: data.total,
    ...
    const dailyTrendArray = Array.isArray(stats.dailyTrend) ? stats.dailyTrend : [];
    ...
    console.log(`[triggerImmediateSync] Successfully synced user ${userId}`);
    return { success: true };
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    console.error(`[triggerImmediateSync] Failed to sync user ${userId}:`, error);
